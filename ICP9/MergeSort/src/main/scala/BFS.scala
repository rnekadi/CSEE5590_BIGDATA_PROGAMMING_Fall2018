...
object BFS {
  ...
  val startCharacterID = 11
  val targetCharacterID = 44
  
  // We make our accumulator a "global" Option so we can reference it in a mapper later.
  var hitCounter:Option[LongAccumulator] = None
  
  // Some custom data types 
  // BFSData contains an array of Node ID connections, the distance, and color.
  type BFSData = (Array[Int], Int, String)
  // A BFSNode has a NodeID and the BFSData associated with it.
  ...
  def createStartingRdd(sc:SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("/Users/sai/Documents/GitHub/CSEE5590_BIGDATA_PROGAMMING_Fall2018/" +
      "ICP9/MergeSort/nodesmall.txt")
  ...
  def main(args: Array[String]) {
    ...
    for (iteration <- 1 to 10) {
      ...
      //iterationRdd.collect().foreach(println)
    }
  }
}